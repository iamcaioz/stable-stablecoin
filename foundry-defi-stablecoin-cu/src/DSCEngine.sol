...
contract DSCEngine is ReentrancyGuard {
    ...
    mapping(address => uint96) private s_weights;
    ...
    function mintDsc(uint256 amountDscToMint) public moreThanZero(amountDscToMint) nonReentrant {
        ...
        uint256 nav = getBasketPrice();
        if (nav < MIN_BASKET_PRICE || nav > MAX_BASKET_PRICE) revert PegDeviationTooLarge();

        // Then mint proportional collateral (one token per basket weight)
    ...
    function _isFullyBacked(uint256 mintAmount) internal view returns(bool) {
        ...
        return basketPrice >= MIN_BASKET_PRICE;
    }
    ...
    */
    ...
    function getBasketPrice() public view returns (uint256 price) {
        for (uint i; s_collateralTokens.length; i++) {
            address token = s_collateralTokens[i];
            (, int256 answer,,,) = AggregatorV3Interface(s_priceFeeds[token]).staleCheckLatestRoundData();
            price += (uint256(answer) * 1e10 * s_weights[token]) / 10_000;
        }
    }
